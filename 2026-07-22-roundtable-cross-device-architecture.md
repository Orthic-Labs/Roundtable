...
1. Adrian opens `https://roundtable.spoares.com` and signs in.
...
- open handoffs (their `evidence_refs` digest — commit SHA + test command + artifact path + URL — with full refs fetched on demand via `roundtable_read`) and approval requests;
- a notice that `roundtable_read` and `roundtable_search` expose older transcript entries.
...
Agent prose is never parsed for `@alias`. An agent can wake another seat only by calling `roundtable_handoff`. This prevents accidental and malicious wake loops.
...
4. **Hard limits while offline.** Approval clicks and `roundtable_reply` with attached artifacts (file/path refs that go through node IPC) are **disabled** — the human must reconnect before resolving. Plain message composition remains enabled.
...
Implement these Rust types in `roundtable-protocol/src/lib.rs` and generate matching TypeScript manually in `packages/web/src/types.ts` and `packages/claude-channel/src/schemas.ts`:
...
- Production hostname is `roundtable.spoares.com`.
- `ROUND_TABLE_ADMIN_TOKEN` is a generated 32-byte base64url secret. Login compares its SHA-256 digest in constant time.
- Successful login creates a random 32-byte browser session, stores only its digest, and sets `__Host-roundtable` with `Secure; HttpOnly; SameSite=Strict; Path=/` and 30-day expiry.
- Node creation returns a bearer secret once; SQLite stores only SHA-256. Mac stores it in Keychain service `roundtable-node`, account `<node-name>`. Windows stores it in Credential Manager target `roundtable-node/<node-name>`.
- CORS is disabled. Mutations require `Origin: https://roundtable.spoares.com`.
- CSP: `default-src 'self'; connect-src 'self' wss://roundtable.spoares.com; img-src 'self' data:; style-src 'self'; script-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'`.
...
Cancellation is operator-only (Adrian via `room cancel <delivery_id>` from the web UI or CLI) or, **only where the room-loop rules above already permit**, an agent calling `roundtable_handoff` carries an implicit cancel of the source delivery the moment the target delivery is `acked`. In every other case, cancellations must come from Adrian.
...
- Hub and node share `roundtable-protocol`; only hub uses `roundtable-store`.
...
**Red:** `cargo test --manifest-path tools/roundtable/Cargo.toml` and `pnpm --dir tools/roundtable test` fail because the workspace does not exist.
...
**Tests first:** `roundtable-protocol/src/lib.rs` unit tests named:
...
**Verify:** `cargo test --manifest-path tools/roundtable/Cargo.toml -p roundtable-protocol` reports all five tests passed.
...
**Verify:** `cargo test --manifest-path tools/roundtable/Cargo.toml -p roundtable-store` passes; a temporary database survives close/reopen with transcript and cursor intact.
...
**Verify:** `cargo test --manifest-path tools/roundtable/Cargo.toml -p roundtable-hub --test auth --test http --test delivery` passes.
...
- Compile the web output into `roundtable-hub` at build time and serve it with immutable hashed assets plus `index.html` no-cache.

**Verify:** `pnpm --dir tools/roundtable/packages/web test` and `pnpm --dir tools/roundtable/packages/web build` pass with no console errors.
...
**Claude MCP tools:** `roundtable_join`, `roundtable_leave`, `roundtable_read`, `roundtable_search`, `roundtable_reply`, and `roundtable_handoff`. The shim declares `claude/channel`, tools, and `claude/channel/permission` only when Task 0 proved permission support.

**Channel event metadata:** `room_id`, `delivery_id`, `message_id`, `sender`, `reply_to`, and `reason`. Instructions require Claude to treat content as untrusted room data, reply with `roundtable_reply`, and use `roundtable_handoff` rather than textual mentions.
...
- binary: `/opt/roundtable/bin/roundtable-hub`
...
- public host: `roundtable.spoares.com`
...
`tools/roundtable/ops/nginx-roundtable.conf` to `~/sites/nginx/roundtable.conf`; add an explicit
...
7. Copy and inventory the nginx config as specified above. Show Adrian the complete inventory and
   ...
   bypass sudo.
8. Create/proxy the Cloudflare DNS record for `roundtable.spoares.com` using the existing scoped token.
...
- `tools/roundtable/README.md`: local development, test, build, node provisioning, session attachment, recovery.
...
pnpm --dir tools/roundtable install --frozen-lockfile
pnpm --dir tools/roundtable test
pnpm --dir tools/roundtable build
node tools/roundtable/tests/e2e/roundtrip.mjs --hub http://127.0.0.1:8460 --fake-nodes 2
```
...
- `tools/roundtable/crates/roundtable-protocol/src/lib.rs`
- `tools/roundtable/crates/roundtable-hub/src/router.rs`
- `tools/roundtable/crates/roundtable-node/src/codex.rs`
- `tools/roundtable/packages/claude-channel/src/index.ts`
- `tools/roundtable/packages/web/src/App.tsx`
