...
export function RoomView({ room, seats, sessions, messages, tasks = [], runs = [], runEvents = {}, runInspections = {}, approvals, online, onSend, onLoadOlder, onAttach, onDetach, onCreateTask = async () => {}, onResolve, onHandoff }: {
  ...
  return <div className="room-view">
    <header className="room-header"><div className="room-id"><div className="room-title-row"><h1>{room.title}</h1><span className="room-slug">#{room.slug}</span></div>{room.objective && <p className="objective" title={room.objective}>{room.objective}</p>}</div><div className="seat-rail" aria-label="Seats in this room">{seats.map((seat) => <span key={seat.id}><i className={`presence ${seat.state}`} />@{seat.alias}</span>)}</div></header>
