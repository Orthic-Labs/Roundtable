#!/usr/bin/env node
// Put citadel.spoares.com and api.spoares.com behind Cloudflare Access, WITHOUT cutting off
// the nodes.
//
...
// every Citadel node offline until someone notices.
//
// Both nodes dial wss://citadel.spoares.com/node/connect. Access intercepts the WebSocket
...
// so Citadel inherits the identity rules you already trust instead of a second, drifting copy.

const ACCOUNT = '03ae77ccd7a07bcbb2dcfde47fa7ba3a';
const API = 'https://api.cloudflare.com/client/v4';
const SOURCE_APP_DOMAIN = 'spoares.com'; // the app whose policy we clone
const BYPASS_PATH = 'citadel.spoares.com/node/connect';
...
const TARGETS = ['citadel.spoares.com'];
...
async function main() {
  ...
  if (byDomain.has(BYPASS_PATH)) {
    ...
    plan.push({
      step: 'bypass',
      domain: BYPASS_PATH,
      app: { name: 'Citadel node WebSocket (bypass)', domain: BYPASS_PATH, type: 'self_hosted', session_duration: '24h' },
      policy: {
