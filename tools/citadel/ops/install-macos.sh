...
#   ssh vendure 'node ~/sites/citadel/tools/citadel/ops/enrol-node.mjs node mac'
#
# Then here:
...
# ---- token -------------------------------------------------------------------
if [ -n "${CITADEL_NODE_TOKEN:-${ROUNDTABLE_NODE_TOKEN:-}}" ]; then
  printf '%s' "${CITADEL_NODE_TOKEN:-$ROUNDTABLE_NODE_TOKEN}" > "$CONFIG_DIR/node.token"
  chmod 600 "$CONFIG_DIR/node.token"
  echo "token: written"
elif [ -f "$CONFIG_DIR/node.token" ]; then
  echo "token: reusing existing $CONFIG_DIR/node.token"
else
  echo "ERROR: no token. Enrol on the box, then re-run with CITADEL_NODE_TOKEN=..." >&2
  exit 1
fi

# ---- config ------------------------------------------------------------------
# node_id is required on a first install; on a re-install the existing config keeps it.
if [ -n "${CITADEL_NODE_ID:-${ROUNDTABLE_NODE_ID:-}}" ]; then
  NODE_ID="${CITADEL_NODE_ID:-$ROUNDTABLE_NODE_ID}"
  ...
  echo "ERROR: no CITADEL_NODE_ID and no existing config" >&2
  exit 1
fi
...
<dict>
    ...
    <dict>
        <key>CITADEL_NODE_CONFIG</key><string>$CONFIG_DIR/config.json</string>
        <key>CITADEL_NODE_TOKEN_FILE</key><string>$CONFIG_DIR/node.token</string>
