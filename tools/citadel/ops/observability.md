...
**None, and that is a decision, not an omission.** Citadel is a single-operator system with a
...
```bash
curl -fsS https://citadel.spoares.com/healthz && pm2 describe citadel-hub | head -20
```
...
```bash
curl -fsS -b "$COOKIE" https://citadel.spoares.com/api/nodes | jq '.nodes[] | {id, name, online}'
...
```bash
pm2 restart citadel-hub && sleep 2 && curl -fsS https://citadel.spoares.com/healthz
```
