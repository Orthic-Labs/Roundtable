...
export function createChannel(opts: ClaudeChannelOptions): { server: McpServer; client: IpcClient } {
  ...
  const server = new McpServer({ name: "citadel-claude-channel", version: "0.1.0" });

  server.tool(
    "citadel_session_join",
    "Join a Citadel room by room_id. Optional seat_id pins the session to a specific seat.",
  ...
  server.tool(
    "citadel_leave",
    "Leave a Citadel room by room_id.",
  ...
  server.tool(
    "citadel_join",
    "Redeem a Citadel invite code to join a Citadel room as a Claude seat.",
