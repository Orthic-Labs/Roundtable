...
async function withHub(fn) {
  const hub = createHub({
    ...
    allowedOrigins: ['https://citadel.spoares.com'],
  });
...
test('auth: cookie is __Host- compliant and attribute order matches', () => {
  assert.equal(
    ...
    '__Host-citadel=abc; Path=/; HttpOnly; SameSite=Strict; Max-Age=100; Secure',
  );
...
test('auth: origin guard allows absent Origin but rejects a foreign one', () => {
  const allowed = ['https://citadel.spoares.com'];
  assert.equal(originAllowed(undefined, allowed), true, 'the node connects without an Origin');
  assert.equal(originAllowed('https://citadel.spoares.com', allowed), true);
...
test('login rejects a wrong token and accepts the right one', async () => {
  await withHub(async (base, hub) => {
    ...
    assert.match(good.headers.get('set-cookie'), /^__Host-citadel=/);
  });
});
...
test('E2E: a node connects over WebSocket and the hub tracks it after node.hello', async () => {
  await withHub(async (base, hub) => {
    ...
    // handshake the real citadel-node binary sends, rather than asserting on a raw upgrade.
