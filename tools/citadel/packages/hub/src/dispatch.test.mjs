...
//
// Frame shapes here match citadel-node's REAL serialization, not a hand-guessed one. Every
...
/**
 * Connects a fake node the way the REAL citadel-node binary does
 * (crates/citadel-node/src/hub.rs::connect_and_drive): open the socket, send node.hello
...
test('E2E: a mention reaches the connected node as a delivery.assign frame', async () => {
  await withHub(async ({ hub, store, room, node, seat, wsBase }) => {
    ...
    // Wrapped under delivery_assign: matches citadel-node's HubEvent::DeliveryAssign, an
...
// ClientCommand::PostMessage does (see crates/citadel-node/src/hub.rs). This frame has no
