// Run with: node --test 'tools/citadel/packages/hub/src/e2e-rust-node.test.mjs'
//
// The definitive test for the whole gap this session closed: a REAL compiled citadel-node
...
// packages/hub/src/ -> packages/hub -> packages -> tools/citadel (the Cargo workspace root)
const RUST_WORKSPACE = fileURLToPath(new URL('../../..', import.meta.url));
const NODE_BINARY = join(
  RUST_WORKSPACE, 'target', 'debug', `citadel-node${process.platform === 'win32' ? '.exe' : ''}`,
);
const FIXTURE_CODEX = join(RUST_WORKSPACE, 'fixtures', 'app-server', 'fake-codex.mjs');

test('E2E: the real compiled citadel-node binary delivers a message to Codex and posts a reply', async (t) => {
  ...
  // `cargo build -p citadel-node` run directly took 0.24s. Requiring a pre-built binary is a
  ...
  if (!existsSync(NODE_BINARY)) {
    t.skip(`citadel-node binary not built at ${NODE_BINARY} — run: cargo build -p citadel-node`);
    return;
  }
  ...
  t.after(async () => {
    // SIGKILL and WAIT. `kill()` sends SIGTERM and returns immediately; citadel-node sits in a
  ...
  // human-readable placeholder like 'adrian' fails citadel-node's deserialization with
  ...
  const lifecycle = agentMessages.find((m) => m.body.startsWith('[citadel-node] turn'));
