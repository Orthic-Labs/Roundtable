...
//
// Route surface mirrors crates/citadel-hub/src/{router,http}.rs. Handlers are ported
...
export function createHub({
  ...
  attachWebSocket(server, (conn, req) => {
    ...
    // down every OTHER connection with it. Found by killing the real citadel-node binary
    ...
    // A real node's connect sequence (crates/citadel-node/src/hub.rs::connect_and_drive)
    ...
    flushDeliveries() {
      ...
      for (const d of store.pendingDispatch()) {
        ...
        const parent = message.reply_to
          ...
          : null;
        // 20 = citadel-protocol::CONTEXT_MAX_MESSAGES. Citadel never injects the full
        ...
        // Wrapped under "delivery_assign": citadel-node deserializes this whole payload
        ...
        // below is required by crates/citadel-node/src/hub.rs's HubEvent::DeliveryAssign; the
