...
## top5
- **** — rank: 1; issue: Implement hub HTTP+WS surface; evidence: tools/citadel/crates/citadel-hub/src/{router,ws}.rs
- **** — rank: 2; issue: Implement node-side Claude + Codex adapters + IPC; evidence: tools/citadel/crates/citadel-node/src/main.rs (empty)
- **** — rank: 3; issue: Merge PWA branch into main and re-run end-to-end smoke; evidence: feat/roundtable-pwa-task6
- **** — rank: 4; issue: Write ops/ (install-macos.sh, install-windows.ps1, nginx.conf, systemd unit, backup.sh, observability.md); evidence: tools/citadel/ops/ does not exist
