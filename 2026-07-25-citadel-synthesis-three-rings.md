...
baseline: 2026-07-22-citadel-cross-device-architecture.md
---
...
|---|---|
| README: hub has "auth/origin guard + HTTP + ws + replay, 24 tests" | `crates/citadel-hub/src/main.rs` is **18 lines**: `/healthz`, `/readyz` |
...
flowchart TB
    ...
    A --- T
    T -->|"seat calls citadel_delegate"| RUN
...
The one new primitive. A seat calls `citadel_delegate` and receives a durable task ID
...
**Phase 2 — durable runs.** Add the Task / Run / RunEvent tables and `citadel_delegate`,
