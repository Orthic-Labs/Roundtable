...
**Decision:** `tools/citadel/packages/hub` is Citadel's single maintained hub spine.

The deployed Node hub owns HTTP, browser WebSocket, SQLite DTO mapping, operator events, & node delivery. `crates/citadel-hub` remains buildable as a frozen recovery/reference implementation, marked by `CITADEL_RUST_HUB_FROZEN`; it receives no routes, features, or contract repairs.
