...
2. Add failing migration/store tests for exactly one task/run, ordered/deduplicated run events, terminal result, and a transaction rollback. Add `0002_task_runs.sql` plus versioned Node/Rust migration application; run `node --test src/store.test.mjs` and `cargo test -p roundtable-store`.
3. Add a failing node-state test that requires a persisted cursor/delivery before ACK. Persist state atomically in the driver and use the received event cursor rather than the handshake cursor; run `cargo test -p roundtable-node`.
...
- tools/roundtable/packages/hub/src/store.mjs
- tools/roundtable/packages/hub/src/server.mjs
- tools/roundtable/crates/roundtable-node/src/hub.rs
- tools/roundtable/crates/roundtable-node/src/main.rs
- tools/roundtable/crates/roundtable-store/migrations/0002_task_runs.sql
